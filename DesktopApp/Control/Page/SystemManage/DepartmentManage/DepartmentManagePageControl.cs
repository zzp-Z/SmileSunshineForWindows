...
namespace DesktopApp.Control.Page.SystemManage.DepartmentManage
{
    public partial class DepartmentManagePageControl : UserControl
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (addForm.ShowDialog() == DialogResult.OK)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    MessageBox.Show($"Adding department failed:{errorMessage}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dataGridViewDepartments.SelectedRows.Count == 0)
            {
                MessageBox.Show("Please select the department to be deleted!", "Prompt", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            var result = MessageBox.Show($"Are you sure you want to delete the selected department {dataGridViewDepartments.SelectedRows.Count}", 
            ...
            if (result == DialogResult.Yes)
            {
                ...
                {
                    string errorMessage = GetUserFriendlyErrorMessage(ex.Message);
                    MessageBox.Show($"Deleting department failed:{errorMessage}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private string GetUserFriendlyErrorMessage(string originalError)
        {
            ...
            if (originalError.Contains("foreign key constraint fails"))
            {
                ...
                {
                    return "Please delete the department role first.";
                }
